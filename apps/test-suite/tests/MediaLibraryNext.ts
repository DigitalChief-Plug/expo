...
export async function test(t) {
  ...
  t.describe('Album creation', () => {
    ...
    t.it('creates an album from a list of assets', async () => {
      ...
      for (const asset of assets) {
        t.expect(
          fetchedAssets.findIndex((fetchedAsset) => fetchedAsset.contentUri === asset.contentUri)
        ).not.toBe(-1);
      }
    });
  ...
  t.describe('Asset creation', () => {
    t.it('creates a PNG asset', async () => {
      ...
      t.expect(asset.contentUri).toBeDefined();
    });
    ...
    t.it('creates an asset inside an album', async () => {
      ...
      t.expect(newAsset.contentUri).toBeDefined();

      const assets = await album.getAssets();
      const assetIds = assets.map((a) => a.contentUri);
      t.expect(assetIds).toContain(newAsset.contentUri);
    });
  });
  ...
  t.describe('Add asset to album', () => {
    t.it('adds an asset to an existing album', async () => {
      ...
      console.log(oldUri, await newAsset.getUri());
      t.expect(oldUri).not.toBe(await newAsset.getUri());
      t.expect(assets.find((asset) => asset.contentUri === newAsset.contentUri)).not.toBe(null);
    });
  });
